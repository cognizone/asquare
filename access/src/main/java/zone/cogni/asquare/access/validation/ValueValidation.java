...
public class ValueValidation {
  ...
  public static Function<Rule, ValidationResult> withoutReport(Model model,
    ...
    return rule -> getValidationResult(new InternalRdfStoreService(model), value, applicationProfile, rule, new ValidationResult.Conforming());
  }

  public static Function<Rule, ValidationResult> withReport(Model model,
                                                            Object value,
                                                            ApplicationProfile applicationProfile) {
    return rule -> getValidationResult(new InternalRdfStoreService(model), value, applicationProfile, rule, new ValidationResult.Reporting());
  }

  public static Function<Rule, ValidationResult> withReport(RdfStoreService rdfStoreService,
                                                            Object value,
                                                            ApplicationProfile applicationProfile) {
    return rule -> getValidationResult(rdfStoreService, value, applicationProfile, rule, new ValidationResult.Reporting());
  }

  private static ValidationResult getValidationResult(RdfStoreService rdfStoreService,
                                                      Object value,
                                                      ApplicationProfile applicationProfile,
                                                      Rule rule,
                                                      ValidationResult validationResult) {
    ValueValidation valueValidation = new ValueValidation(validationResult, applicationProfile, rdfStoreService, value);
  ...
  private ValueValidation(ValidationResult validationResult,
                          ApplicationProfile applicationProfile,
...
//      if (model.contains(node.asResource(), RDF.type, model.createResource(rdfType.getValue()))) return new RdfStatements();
      if (ask(node.asResource().getURI(), RDF.type.getURI(), rdfType.getValue())) return new RdfStatements();
  ...
  private boolean ask(String uri, String property, String resource) {
    String ask = "ASK { " +
                 "   <" + uri + ">  <" + property + ">  <" + resource + ">. " +
                 "} ";

    return rdfStoreService.executeAskQuery(ask);
  }
...
//      if (model.contains(node.asResource(), SKOS.inScheme, model.createResource(inScheme.getValue()))) return new RdfStatements();
      if (ask(node.asResource().getURI(), SKOS.inScheme.getURI(), inScheme.getValue())) return new RdfStatements();
...
//      if (model.contains(model.createResource(memberOf.getValue()), SKOS.member, object)) return new RdfStatements();
      if (ask(memberOf.getValue(), SKOS.member.getURI(), node.asResource().getURI())) return new RdfStatements();
  ...
  private Function<LiteralValue, RdfStatements> validateLiteralValue() {
    return literalValue -> {
      ...
      Literal expectedLiteral = RdfParserUtils.parseLiteral(literalValue.getValue());
...
//      if (model.contains(resource, model.createProperty(PropertyPathUriMapper.getUri(propertyPath.getPath())), RdfParserUtils.parseRdfNode(propertyPath.getValue()))) return new RdfStatements();
      Preconditions.checkState(node.asResource().isURIResource());
      // todo
      if (ask(node.asResource().getURI(), ResourceFactory.createProperty(PropertyPathUriMapper.getUri(propertyPath.getPath())).getURI(), RdfParserUtils.parseRdfNode(propertyPath.getValue()).toString())) return new RdfStatements();
