...
/**
 *  todo add and implement method to retrieve all resources for a given set of uri's on access layer! performance {@code ->} e.g. only one query in elastic
 *  todo support find by attribute {@code ->} find all possible values for a given attribute (e.g to support select boxes)
 *  todo attributes representing relations {@code ->} inverse indexing to load and retrieve related documents (resources) directly (~ tagging properties)
