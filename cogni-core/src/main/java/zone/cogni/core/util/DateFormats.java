...
public class DateFormats {
  ...
  public static Date parseXsdDateTimeFormat(String date) {
    if (date.startsWith("-")) {
      log.error("Our conversions are of by one year on negative dates: {}", date);
    }
    ...
    try {
      ...
      try {
        ...
        // special formats
        Date result = DateFormats.parse(date, Format.XMLDATETIME);
        log.warn("DateTime format with undefined timezone: {}", date);
        if (result != null) return result;

        throw new IllegalArgumentException("Date conversion for " + date + " not supported?!");
      }
    }
  }
